    ...
    config match {
      case fEOptConfig: FixedEffectOptimizationConfiguration =>
        (interceptIndexOpt: Option[Int]) =>
          DistributedObjectiveFunction(fEOptConfig, lossFunction, treeAggregateDepth, interceptIndexOpt)

      case rEOptConfig: RandomEffectOptimizationConfiguration =>
        (interceptIndexOpt: Option[Int]) =>
          SingleNodeObjectiveFunction(rEOptConfig, lossFunction, interceptIndexOpt)
